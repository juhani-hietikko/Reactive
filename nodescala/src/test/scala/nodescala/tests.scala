...
class NodeScalaSuite extends FunSuite {
  ...
  test("Test delay") {
    val future = Future.delay(100 millis)
    Await.ready(future, 110 millis)
    assert(future.isCompleted)
  }
  
  test("Test delay and any") {
    val future1 = Future.delay(100 millis)
    val future2 = Future.delay(200 millis)
    val futureList = List(future1, future2)

    val anyFuture = Future.any(futureList)

    Await.ready(anyFuture, 150 millis)
